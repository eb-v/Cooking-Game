...
public class GrabSystem : ScriptableObject
{
    ...
    public static void GrabObject(GameObject player, Rigidbody rb, GrabData grabData)
    {
        if (!SystemEnabled)
            return;
        ...
        CoroutineRunner.Instance.StartCoroutine(GrabObjectCoroutine(rc, rb, grabData));
    }

    private static IEnumerator GrabObjectCoroutine(RagdollController rc, Rigidbody rb, GrabData grabData)
    {
        ...
        rb.transform.position = body.transform.TransformPoint(grabData.position);
        rb.transform.rotation = body.transform.rotation * grabData.rotation;
        lowerRightArm.AddComponent<FixedJoint>().connectedBody = rb;
        lowerLeftArm.AddComponent<FixedJoint>().connectedBody = rb;
    }
